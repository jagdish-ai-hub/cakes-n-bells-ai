...
const ProductDetail: React.FC<ProductDetailProps> = ({ wishlist, onToggleWishlist }) => {
  ...
  return (
    <div className="pb-24 animate-pop-in relative bg-white min-h-screen md:bg-pink-50/20 md:py-12 md:pb-16">
      {/* Back Button */}
      <button 
        onClick={() => navigate(-1)}
        className="fixed md:absolute top-4 left-4 z-20 w-10 h-10 bg-white/90 backdrop-blur rounded-full shadow-lg flex items-center justify-center text-gray-700 active:scale-90 transition-all border border-pink-50 md:hover:bg-pink-50"
      >
        <i className="fas fa-chevron-left"></i>
      </button>

      <div className="max-w-5xl mx-auto md:bg-white md:rounded-[2.5rem] md:shadow-xl md:border md:border-pink-50/50 md:overflow-hidden relative">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0 md:gap-12 md:p-8">
          
          {/* Swipeable Image Gallery */}
          <div className="relative aspect-[4/5] overflow-hidden bg-gray-100 md:rounded-3xl md:shadow-md md:border md:border-pink-50/50">
            <div 
              ref={scrollRef}
          ...
          {/* Product Content */}
          <div className="px-6 py-8 md:px-0 md:py-4 flex flex-col justify-between h-full">
            <div>
              <div className="flex justify-between items-start mb-4">
                <div className="pr-4">
                  <span className="text-pink-500 font-bold text-[10px] tracking-[0.2em] uppercase mb-1 block">{product.category}</span>
                  <h1 className="text-3xl font-black text-gray-900 font-serif leading-none tracking-tight md:text-4xl">{product.name}</h1>
                </div>
              ...
              <div className="space-y-8">
                ...
                {weights.length > 0 && (
                  <div className="animate-fade-in-up delay-100">
                    <label className="block text-[11px] font-black text-gray-400 uppercase tracking-widest mb-3">Choose Weight</label>
                    <div className="flex space-x-3">
                      {weights.map((w) => (
                        <button
                          key={w}
                          onClick={() => setSelectedWeight(w)}
                          className={`flex-1 py-3 px-4 rounded-2xl border-2 transition-all font-bold text-sm ${
                            selectedWeight === w 
                ...
                <div className="animate-fade-in-up delay-200">
                  <label className="block text-[11px] font-black text-gray-400 uppercase tracking-widest mb-3">How many?</label>
                  <div className="flex items-center space-x-8 bg-pink-50/30 w-fit px-5 py-2 rounded-2xl border border-pink-100/50">
                    <button 
                      onClick={() => setQuantity(Math.max(1, quantity - 1))}
                      className="w-8 h-8 rounded-lg bg-white flex items-center justify-center text-pink-500 shadow-sm active:scale-75 transition-transform"
                    >
                      <i className="fas fa-minus text-xs"></i>
                    </button>
                    <span className="font-black text-lg text-gray-800 min-w-[30px] text-center">{quantity}</span>
                    <button 
            ...
            {/* Desktop Actions */}
            <div className="hidden md:block mt-8 pt-6 border-t border-pink-100/40">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Total Price</span>
                  <div className="text-3xl font-black text-pink-600 tracking-tight">₹{totalPrice}</div>
                </div>
              </div>
              <button 
                onClick={handleBuyNow}
                className="w-full cake-pink text-white py-4 px-8 rounded-2xl font-black shadow-xl shadow-pink-200 active:scale-[0.97] transition-all flex items-center justify-center space-x-3 hover:opacity-95"
              >
      ...
      {/* Sticky Bottom Nav */}
      <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-pink-50 p-5 z-40 shadow-[0_-10px_30px_rgba(0,0,0,0.03)] md:hidden">
